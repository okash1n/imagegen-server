...
- キューは揮発。完了結果(画像 + メタ)はディスク永続のため、再起動してもギャラリーは残る。実行中・失敗のジョブ記録はインメモリのみで、再起動すると一覧から消える(v1 注記: 揮発キューでは failed マークを永続できないため「failed 扱いで残す」は行わない)
...
- GUI 経由の参照画像はサーバーの管理ディレクトリ(uploads / images)内に配置した上で絶対パスを渡す。REST / MCP を直接呼ぶ場合は、存在 + 画像拡張子を検証した任意の絶対パスを許容する(read-only sandbox からの読み取り可否は実機検証項目に含める)

### 5.4 保存とメタデータ

- 画像: `~/.imagegen-server/images/<job_id>.png`
- メタ: `~/.imagegen-server/images/<job_id>.json`
  - prompt、種別(generate / edit)、参照画像パス、所要時間、生成日時、使用エンジン
  - v1 注記: 失敗ジョブのメタは永続化しない(失敗はインメモリのジョブ一覧と GUI/MCP のエラー表示でのみ可視)
- ギャラリー = メタファイルのスキャン。DB は使わない
- 設定: `~/.imagegen-server/config.json`(port、同時実行数、ターン用モデル、codex バイナリパス)。CLI フラグで上書き可。保存先(dataDir)は config.json 自体の所在が依存するため `--data-dir` フラグのみで指定する
